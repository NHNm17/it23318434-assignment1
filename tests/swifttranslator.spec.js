...
test('Pos_Fun_0005 – Convert mixed English terms', async ({ page }) => {
  ...
  await inputBox.fill('adha mata office ekea Zoom meeting ekak thiyenavaa.');

  await expect(outputBox).toContainText(
    'අද මට office එකේ Zoom meeting එකක් තියෙනවා.',
    { timeout: 15000 }
  );
});
...
test('Pos_Fun_0008 – Convert polite inquiry', async ({ page }) => {
  ...
  await inputBox.fill('oyage nama mokkadha?');
...
test('Pos_Fun_0009 – Convert compound future tense', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('api pasalata yanavaa saha passe teachers laa hambenavaa');
  await expect(outputBox).toContainText(
    'අපි පසලට යනවා සහ පස්සෙ teachers ලා හම්බෙනවා',
    { timeout: 15000 }
  );
});
...
test('Pos_Fun_0011 – Convert present tense', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('apee paeththee innavaa e minissu.');
  await expect(outputBox).toContainText(
    'අපේ පැත්තේ ඉන්නවා එ මිනිස්සු.',
    { timeout: 15000 }
  );
});
...
test('Pos_Fun_0012 – Convert polite suggestion', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('oyage atha balanna');
  await expect(outputBox).toContainText(
    'ඔයගෙ අත බලන්න',
    { timeout: 15000 }
  );
...
test('Pos_Fun_0014 – Convert future sentence', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('adha office ekata yanavaa mama.');
  await expect(outputBox).toContainText(
    'අද office එකට යනවා මම.',
    { timeout: 15000 }
  );
});
...
test('Pos_Fun_0016 – Convert sentence with number', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('mama kello 3 k ekka innavaa.');
  await expect(outputBox).toContainText(
    'මම කෙල්ලො 3 ක් එක්ක ඉන්නවා.',
    { timeout: 15000 }
  );
});
...
test('Pos_Fun_0017 – Convert sentence with name', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('nimal gedhara gihin poLata yanava.');
  await expect(outputBox).toContainText(
    'නිමල් ගෙදර ගිහින් පොළට යනව.',
    { timeout: 15000 }
  );
});
...
test('Pos_Fun_0018 – Convert slang with Question', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('machan uba kiiyatadha yannee?');
  await expect(outputBox).toContainText(
    'මචන් උබ කීයටද යන්නේ?',
    { timeout: 15000 }
  );
...
test('Pos_Fun_0024 – Convert polite morning greeting to teacher', async ({ page }) => {
  ...
  const outputBox = page
    .locator('div.whitespace-pre-wrap.overflow-y-auto')
    .first();
  await inputBox.fill('suba udhaesanak teacher obata kohomadha?');
  await expect(outputBox).toContainText(
    'සුබ උදැසනක් teacher ඔබට කොහොමද?',
    { timeout: 15000 }
  );
});
...
test('Neg_Fun_0003 – Fail with random abbreviations', async ({ page }) => {
  ...
  await inputBox.fill('ATM PIN ekee dunnam');
